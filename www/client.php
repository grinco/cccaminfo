<?php
   ...
   {
   	if (isset($UsageUsers[$username]))
   	{
   		$lastSeenOnline  = $UsageUsers[$username]["time"];
   		$ac_IP         = $UsageUsers[$username]["ip"];
   		
			$text_offline = get_formatted_timediff($lastSeenOnline). " ago";
   		$ac_Connected  = "<FONT COLOR=red>".$text_offline."</FONT>";
			$ac_Idle 		= "-";
			$acEcm 			= "";
			$acEcmOk 		= "";
			$acEmm			= "";
   ...
   else
   {
	  ...
	  $indexclient = $lastIndexEcm;
	   ...
	   $ecm_total = 0;
	   $ecmOK_total = 0;
	   if (isset($clientConectat[$username]["Info"]["Idents"]))
